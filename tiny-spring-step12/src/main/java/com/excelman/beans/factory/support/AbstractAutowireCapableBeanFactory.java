...
import com.excelman.beans.factory.config.*;
...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
    ...
    /**
     * 创建bean方法的实现：
     * 1. step12新增的：在创建bean之前，判断是否返回代理bean对象
     * 2. 首先由bean的定义，获取bean对应的class，并进行实例化
     * 3. 给bean填充属性 （!!!step12新增：注意有个大坑！！因为实现实例化，再填充属性。所以，对于构造的bean，必须要有空参数的构造函数!!!）
     * 4. step6新增的：调用初始化方法，执行BeanPostProcessor的前置和后置处理
     * 5. step7新增的：注册实现了DisposableBean接口的Bean对象，将销毁方法保存起来，方便后续执行销毁动作
     * 6. step9新增的：判断bean是否是单例的，若是的话，才添加到单例的内存中
     */
    @Override
    ...
    /**
     * 解析是否是代理对象，若是的话，最终返回该bean的代理对象，否则返回null
     */
    ...
    /**
     * 执行初始化bean
     * step8新增的：首先判断当前bean是否是Aware类型，若是的话则执行响应的方法
     * 调用BeanPostProcessor Before和After进行初始化的前后置处理
     * @param beanName
     * @param bean
     * @param beanDefinition
     * @return
